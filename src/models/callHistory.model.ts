...
callHistoryModel.getCallhistoryByUid = (uid: string, callCollection: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      console.log('get call history');
      const res = callCollection.findOne({ uuid: uid });
      resolve(res);
    } catch (err) {
      console.log(err);
      reject(err);
...
/**
 * Delete call history by uuid
 * @param uuid  - unique id
 * @param callCollection - call collection
 */
callHistoryModel.deleteCallHistory = (uuid: string, callCollection: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      console.log('delete call history');
      const res = callCollection.deleteMany({ uuid: uuid });
      resolve(res);
    } catch (err) {
      console.log(err);
      reject(err);
